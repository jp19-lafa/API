...
module.exports = {
  ...
  AuthController : class {
    constructor(services) {
      ...
      services.models.user.findOne({email : 'admin@farmlab.team'})
          .exec((error, user) => {
            if (!user) {
              ...
              user.save().then(
                  result => { services.logger.info('Created Global Admin'); })
            }
          })
    }
    ...
    async authenticateUserByCredentials(email, password) {
      return new Promise(
          (resolve,
           reject) => {this.services.models.user.findOne({email : email})
                           .then(user => {
    ...
    async authenticateUserByRefreshToken(token) {
      return new Promise(
          (resolve,
           reject) => {this.services.models.user.findOne({refreshToken : token})
    ...
    _generateJWT(sub, options = {}) {
      return jwt.sign({audience : "aud:*", issuer : "FarmLabTeam", sub : sub},
                      this.services.keys.private, {expiresIn : "24h"});
    }
  }
