  ...
  MqttController : class {
    constructor(services) {
      ...
      services.mqtt.authenticate = (client, username, password, callback) => {
        if (!client.id.match(/^([0-9A-Fa-f]{2}[:]){5}([0-9A-Fa-f]{2})$/))
          callback(null, false);
        Node.findOne({macAddress : username, authorizationKey : password})
            .exec((error, node) => {
              if (!node || error) {
                callback(null, false);
    ...
    handleSensorMessage(packet, client) {
      ...
      const sensorDataPoint =
          new DataPoint({value : parseFloat(packet.payload.toString())});

      sensorDataPoint.save();
      Node.findOneAndUpdate({macAddress : client.id}, {
            ['sensors.' + packet.topic.split("/")[2] + '.value'] :
                sensorDataPoint.value,
            ['sensors.' + packet.topic.split("/")[2] + '.timestamp'] :
                sensorDataPoint.timestamp,
            $push : {
              ['sensors.' + packet.topic.split("/")[2] + '.history'] :
                  sensorDataPoint
            }
          })
          .exec((error, node) => {
            if (error)
              return this.services.logger.error(error);
            this.services.logger.info(
