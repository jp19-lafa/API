...
module.exports = {
  ...
  MqttController: class {
    ...
    constructor(services) {
      ...
      services.mqtt.authenticate = (client, username, password, callback) => {
        if (!client.id.match(/^([0-9A-Fa-f]{2}[:]){5}([0-9A-Fa-f]{2})$/))
          callback(null, false);
        services.models.node.findOne({ macAddress: username, authorizationKey: password }).exec((error, node) => {
          if(!node || error) {
            callback(null, false);
      ...
      services.mqtt.on('clientDisconnected', function (client) {
        ...
        services.models.node.findOne({ macAddress: client.id }).exec((error, node) => {
          if(!node || error)
            return;
    ...
    handleSensorMessage(packet, client) {
      this.services.logger.info(`Received: [SENSOR] ${packet.payload.toString()} for ${packet.topic.split('/')[2]} on ${client.id}`);
    }
