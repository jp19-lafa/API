...
module.exports = (services) => {
  ...
  router.get('/:id', [
    ...
    services.models.node.findOne({ _id: req.params.id, members: req.user.sub }).select('-__v -authorizationKey').populate({ path: 'members', select: '_id firstname lastname' }).exec((error, node) => {
      if (error || !node)
  ...
  router.put('/:id/actuator', [
    param('id').isMongoId().trim().escape(),
    body('actuator').isIn(services.types.actuators).trim().escape(),
    body('state').isNumeric().trim().escape()
    // .custom(value => {
    //   console.log(value);
    //   if (parseInt(value) > 255 || parseInt(value) < 0) return Promise.reject('Invalid state value');
    // })
  ], async (req, res) => {
