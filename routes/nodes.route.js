...
module.exports = services => {
  router.get("/testnode", async (req, res) => {
    ...
    nodeM.save().then(node => { res.send({amount : 0, result : node}); });
  });

  router.get(
      "/:id", [ param("id").isMongoId().trim().escape() ], async (req, res) => {
             ...
             async (req, res) => {
               ...
               services.models.node
                   ...
                   .exec((error, node) => {
                     ...
                     const packet = {
                       topic :
                           `${node.macAddress}/actuators/${req.body.actuator}`,
                     ...
                     services.logger.info(
                         `Published: [ACTUATOR] ${req.body.state} for ${
                             req.body.actuator} to ${node.macAddress}`);
